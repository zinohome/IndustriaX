...
**MVP 范围（已定）：worker 启停/重启一步到位进 UI** —— 部署管理 MVP 含观测、编排包激活/回滚、**以及 worker（及其它 compose 服务）的启动/停止/重启**。这要求一个配套控制面，设计见下。

#### 5.2.1 部署控制服务（deploy-control，P5 新增组件）

Web UI 不直接碰容器运行时；启停/重启经 API Gateway 路由到一个**最小化、白名单约束的部署控制服务**，由它对 docker/compose 执行生命周期动作。

- **白名单作用域**：只能操作 IndustriaX compose 工程内的已知服务（activity-worker / workflow-worker / 各 MCP / temporal / ollama / postgres / ragflow…），**禁止任意容器或任意 shell**——这是攻击面收口的硬约束。
- **RBAC**：启停/重启属最高权限运维角色；普通角色只读部署状态。
- **审计**：每个动作记 who / 操作哪个服务 / 动作 / 触发时间 / 结果，进合规审计日志。
- **二次确认**：破坏性动作（停止/重启平台运行时服务）UI 二次确认；批量/全栈动作显式拦截。

#### 5.2.2 优雅停机与影响面（启停语义，不能糊）

启停不是 `kill -9`，要按服务语义优雅处理，UI 须把影响面显式告诉操作者：

- **workflow-worker 重启**：影响面小。workflow 状态由 Temporal 持久化，重启只是暂停 poll，恢复后续跑；优先用于"换编排包版本"。
- **activity-worker 重启**：进行中的 activity 会按 Temporal **at-least-once** 重试——已由 P0 冻结的 `idempotency_key` + 幂等账本兜住，不丢不重复执行副作用。
- **优雅 drain**：停 worker 走 SIGTERM + worker shutdown timeout，先排空在途任务再退出，避免硬切；UI 显示 drain 进度。
- **平台有状态服务（postgres/temporal）**：默认不暴露一键重启给常规运维，或加更强二次确认 + 维护窗口提示（数据底座抖动影响全栈）。

**横切纪律（与产品既有 RBAC/审计一致）：** 所有部署动作（激活/回滚/启停/重启/触发迁移）走 RBAC 高权限角色 + 写审计日志（合规证据）+ 破坏性动作二次确认 + 优雅 drain。
